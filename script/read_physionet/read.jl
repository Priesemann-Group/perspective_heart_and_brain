...
"""
    plot_ecg(signal::Vector{Float64}, fs::Float64) -> Figure

Plots the given ECG signal (assumed to be in physical units, e.g. mV) versus time.
The time axis is computed from the sampling frequency.
"""
function plot_ecg(signal::Vector{Float64}, fs::Float64)
    n = length(signal)
    # Create time axis (in seconds)
    t = range(0, length=n, step=1/fs)
    fig = Figure(resolution = (800, 400))
    ax = Axis(fig[1, 1], xlabel = "Time (s)", ylabel = "ECG (mV)", title = "ECG Fragment")
    lines!(ax, t, signal, color = :blue, linewidth = 2)
    return fig
end
...
"""
    main(record_path::String)

Reads the WFDB record from the given base record path and plots the first (or only)
ECG signal using CairoMakie.
"""
function main(record_path::String)
    println("Reading WFDB record from: $record_path")
    header, signal_or_signals = read_wfdb_record(record_path)
    println("Record name: ", header.record_name)
    println("Number of signals: ", header.nsig)
    println("Sampling rate: ", header.fs, " Hz")
    
    # For this example, we only plot the first channel.
    signal = header.nsig == 1 ? signal_or_signals : signal_or_signals[1]
    println("Number of samples in plotted channel: ", length(signal))
    
    fig = plot_ecg(signal, header.fs)
    display(fig)
end

# --- Script entry point ---

# If no command-line argument is given, use the provided example record.
record_path = length(ARGS) == 0 ?
    "/path/to/ecg-fragment-database-for-the-exploration-of-dangerous-arrhythmia-1.0.0/1_Dangerous_VFL_VF/frag/426_C_VF_454_5s_frag" :
    ARGS[1]
main(record_path)
